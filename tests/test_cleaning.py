...
def test_merge_keep_columns_allowlist():
    params = dict(PARAMS)
    params["keep_columns"] = {
        "demo": ["SEQN", "RIDAGEYR"],          # excluye RIAGENDR/RIDRETH3/INDFMPIR
        "diq": ["SEQN", "DIQ010"],
    }
    merged = merge_nhanes_tables(*_raw(), params)
    assert "RIAGENDR" not in merged.columns      # filtrada por allowlist
    assert "RIDETH3" not in merged.columns
    assert {"RIDAGEYR", "DIQ010", "BMXBMI"}.issubset(merged.columns)  # bmx sin allowlist => completa
